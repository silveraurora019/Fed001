...
from util.options_v1 import args_parser
from util.local_training_v1 import LocalUpdate, globaltest
from util.fedavg import FedAvg, FedAvgWeighted
from util.util_v1 import add_noise, global_sub_prototype_distance, get_output, selective_pseudo_labeling
...
"""
Major framework of noise FL (Version 1: FedLabel-inspired correction in Stage 1)
"""
...
if __name__ == '__main__':
    ...
    txtpath = rootpath + 'txtsave/V1_%s_%s_NL_%.1f_LB_%.1f_Iter_%d_lr_%.2f_BetaP_%.2f_Seed_%d' % (
    ...
    print("\n" + "="*25 + " Stage 1: Pre-processing (v1) " + "="*25, flush=True)
    for iteration in range(args.iteration1):
        ...
        # 1. Client-side local training and calculation of local sub-prototypes
        ...
        # 2. Server performs fairness-weighted aggregation
        # 2a. Calculate contribution weights
        ...
        for client_id in noisy_set:
            ...
            if local_w is not None:
                ...
                # The selective_pseudo_labeling function returns the corrected labels for the entire dataset
                corrected_labels_all = selective_pseudo_labeling(args, net_local, netglob, dataset_train, client_indices)
                
                # Only update the labels for the current client
    ...
    if args.fine_tuning:
        ...
        # selected_clean_idx = np.where(estimated_noisy_level <= args.clean_set_thres)[0]   # 选择所有客户端
        ...
        if len(selected_clean_idx) > 0:
            ...
            for rnd in range(args.rounds1):
                ...
                for idx in idxs_users: # Iterate over all clean clients
                    local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
                ...
                # Use standard FedAvg based on data volume during the finetuning stage
            ...
            if args.correction:
                print("\nPerforming label correction after finetuning...", flush=True)
                y_train_noisy_new = np.array(dataset_train.targets)
                corrected_count_total = 0
                
                for idx in noisy_set:
                    sample_idx = np.array(list(dict_users[idx]))
                    loader = torch.utils.data.DataLoader(Subset(dataset_train, sample_idx), batch_size=100, shuffle=False)
                    glob_output, _ = get_output(loader, netglob.to(args.device), args, False, None)
                    ...
                    if len(relabel_idx) > 0:
                        original_labels = y_train_noisy_new[sample_idx[relabel_idx]]
                        new_labels = y_predicted[relabel_idx]
                        
                        num_changed = np.sum(original_labels != new_labels)
                        corrected_count_total += num_changed

                        y_train_noisy_new[sample_idx[relabel_idx]] = new_labels
                
                dataset_train.targets = y_train_noisy_new
                print(f"Total labels corrected in Stage 2: {corrected_count_total}", flush=True)
    ...
    final_clean_set = np.where(estimated_noisy_level <= args.clean_set_thres)[0]
