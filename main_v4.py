...
from util.options_v4 import args_parser
from util.local_training_v4 import LocalUpdate, globaltest
from util.fedavg import FedAvg, FedAvgWeighted
from util.util_v4 import (add_noise, global_sub_prototype_distance, get_output, 
                          selective_pseudo_labeling, prototype_based_correction, calculate_global_prototypes)
from util.dataset import get_dataset
from model.build_model_v5 import build_model # Assuming model is compatible

np.set_printoptions(threshold=np.inf)

"""
Major framework of noise FL (Version 4: Inter-Client Prototype Contrastive Learning)
"""
...
if __name__ == '__main__':
    ...
    txtpath = rootpath + 'txtsave/V4_%s_%s_NL_%.1f_LB_%.1f_Iter_%d_lr_%.2f_BetaP_%.2f_K_%d_Seed_%d' % (
    ...
    multi_client_prototypes = {} # Initialize the prototype pool

    print("\n" + "="*25 + " Stage 1: Pre-processing with MCPCL " + "="*25, flush=True)
    for iteration in range(args.iteration1):
        ...
        for idx in idxs_users:
            ...
            # === MODIFIED v8: Pass multi-client prototypes to local training ===
        ...
        # === MODIFIED v8: Rebuild multi-client prototypes for the next round ===
        print("\n  <-- Rebuilding Multi-Client Prototypes -->", flush=True)
        ...
        for label in range(args.num_classes):
            ...
            # We just collect all prototypes, not re-cluster them on the server
            if collected_sub_protos:
                multi_client_prototypes[label] = np.array(collected_sub_protos)
    ...
    # ... (Stage 3 remains unchanged from v4)
    final_accuracies = []
